...
class MultiSERModel(BertPreTrainedModel):
	def __init__(self, config):
		atype_emb_dim = 30
		super(MultiSERModel, self).__init__(config)
		self.atype_embedding = nn.Embedding(len(ATYPE_LIST), embedding_dim=atype_emb_dim)
		self.bert = BertModelPlus(config)
		self.dropout = nn.Dropout(p=config.hidden_dropout_prob)
		self.atype_classifier = nn.Linear(config.hidden_size, len(ATYPE_LIST))
		self.classifier = nn.Linear(config.hidden_size + atype_emb_dim, 1)
	...
	def forward_nn(self, batch):
		_, question_emb = self.bert(batch['question_ids'])
		atype_logit = self.atype_classifier(question_emb)
		atype = torch.argmax(F.softmax(torch.sigmoid(atype_logit), dim=-1), dim=-1)
		atype_emb = self.atype_embedding(atype)
		...
		concat = torch.cat((q_poolout, atype_emb), -1)
		dr_pooled_output = self.dropout(concat)
		logits = self.classifier(dr_pooled_output)
		logits = logits.squeeze(-1)
		return logits, atype_logit.squeeze(-1)
	
	def forward(self, batch):
		logits, atype_logit = self.forward_nn(batch)
		crss_entropy_lss = nn.CrossEntropyLoss()
		loss = self.criterion(logits, batch['label'].float()) + \
		       crss_entropy_lss(atype_logit, batch['atype_label'].long())
		return loss
	...
	def predict_fgc(self, batch, threshold=0.5):
		logits, atype_logits = self.forward_nn(batch)
		scores = torch.sigmoid(logits)
		scores = scores.cpu().numpy().tolist()
		atype = torch.argmax(torch.softmax(torch.sigmoid(atype_logits), dim=-1), dim=-1)
		atype = atype.cpu().numpy().tolist()
