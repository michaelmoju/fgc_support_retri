...
from ..dataset_reader import cross_sent_reader as reader
...
class CrossSentModel(BertPreTrainedModel):
    def __init__(self, config):
        super(CrossSentModel, self).__init__(config)
        self.bert = BertModelPlus(config)
        self.dropout = nn.Dropout(p=config.hidden_dropout_prob)
        self.sent_pos_embedding = nn.Embedding(reader.WINDOW * 2, 20)
        self.classifier = nn.Linear(config.hidden_size + 20, 1)
    ...
    def forward_nn(self, batch):
        ...
        sent_pos_emb = self.sent_pos_embedding(batch['position_id'])
        dr_pooled_output = self.dropout(q_poolout)
        concat = torch.cat((dr_pooled_output, sent_pos_emb), dim=-1)
    ...
    def predict_fgc(self, q_batch, threshold=0.5):
        ...
        max_score = 0
        prediction_set = set()
        for score, sent_id in zip(scores, q_batch['sent_id']):
            if score > max_score:
                max_i = sent_id
                max_score = score
            if score >= threshold:
